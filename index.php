...
<body>
  ...
  <script>
    ...
    async function loadStats(){
      ...
      let total=0,male=0,female=0,ponpesCounts={};
      ...
      for(let i=1;i<rows.length;i++){
        ...
        tr.innerHTML=`
          <td>${total}</td><td>${sekolah}</td><td>${nama}</td><td>${alamat}</td>
          <td>${rows[i][colGender]||""}</td><td>${pondok}</td><td>${hp}</td>
          <td>${statusBerkas}</td>
          <td>${hp?`<a href="${linkWA}" target="_blank" onclick="markContacted('${hp}')">WA</a>`:"-"} ${contacted}</td>
        `;
    ...
    function drawCharts(data){
      ...
      genderChartInstance = new Chart(document.getElementById("genderChart"),{
        type:"doughnut",
        data:{labels:["Laki-Laki","Perempuan"],datasets:[{data:[male,female],backgroundColor:["#3b82f6","#ec4899"]}]}
      });
      ponpesChartInstance = new Chart(document.getElementById("ponpesChart"),{
        type:"doughnut",
        data:{labels:Object.keys(ponpesCounts),datasets:[{data:Object.values(ponpesCounts),backgroundColor:["#10b981","#f59e0b","#3b82f6","#ef4444","#8b5cf6"]}]}
      });
    }
    ...
    function renderTable(data){
      ...
      data.forEach((d,i)=>{
        ...
        const tr = document.createElement("tr");
        tr.innerHTML=`
          <td>${i+1}</td><td>${d.sekolah}</td><td>${d.nama}</td><td>${d.alamat}</td>
          <td>${d.gender}</td><td>${d.pondok}</td><td>${d.hp}</td>
          <td>${d.statusBerkas}</td>
          <td>${d.hp?`<a href="${d.linkWA}" target="_blank" onclick="markContacted('${d.hp}')">WA</a>`:"-"} ${contacted}</td>
        `;
        tbody.appendChild(tr);
      });
    }
